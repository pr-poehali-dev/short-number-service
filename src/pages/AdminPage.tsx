...
export default function AdminPage() {
  ...
  function handleEnEdit(id: number) {
    const enItem = enNumbers.find((e) => e.id === id) ?? { id, name: "", description: "", procedure: "" };
    setEnEditingId(id);
    setEnEditForm({ ...enItem });
  }

  function handleEnSave() {
    if (!enEditForm) return;
    const updated = enNumbers.some((e) => e.id === enEditForm.id)
      ? enNumbers.map((e) => (e.id === enEditForm.id ? enEditForm : e))
      : [...enNumbers, enEditForm];
    setEnNumbers(updated);
    saveNumbersEn(updated);
    setEnEditingId(null);
    setEnEditForm(null);
    setSaved(true);
  }
  ...
  const enFiltered = numbers.filter((n) => {
        ...
        {adminTab === "en" && (
          <>
            <div className="flex items-center justify-between mb-4 gap-3">
              ...
              <button
                onClick={() => { if (confirm("Reset all English translations to defaults?")) handleEnReset(); }}
                ...
                return (
                  <div key={num.id} className="bg-white border border-border rounded-xl overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 px-4 py-3 border-b border-border bg-muted/30">
                      ...
                      <button
                        onClick={() => isEditing ? setEnEditingId(null) : handleEnEdit(num.id)}
